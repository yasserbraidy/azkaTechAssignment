...
import { sidebar_color, sidebar_show, sidebar_type, sidebar_menu_style } from "../../../../store/setting/selectors";

// Redux Selector / Action
import { useSelector } from "react-redux";
import ISideBarStyle from "./interface";
import SidebarBorderProfileCard from './../../components/sidebar/sidebar-border-profile-card/index';

const Sidebar = memo((props: ISideBarStyle) => {
  const sidebarColor = useSelector(sidebar_color);
  const sidebarHide = useSelector(sidebar_show); // array
  const sidebarType = useSelector(sidebar_type); // array
  const sidebarMenuStyle = useSelector(sidebar_menu_style);
  ...
  useEffect(() => {
    const element = document.querySelector("#my-scrollbar") as HTMLElement;
    if (element !== null) {
      ...
      >
        <div className="sidebar-header d-flex align-items-center justify-content-start">
          <Link to="/" className="navbar-brand">
            <Logo />
            <h4 className="logo-title">{props.name}</h4>
          </Link>
