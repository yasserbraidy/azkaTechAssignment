...
import IColorCustomization from './interface'
import { IThemeColor } from '../../../../store/setting/interface'

import { themes } from '../../../../utilities/themes'

const ColorCustomizer = memo((props: IColorCustomization) => {
    ...
    const updateColor = (themeColor: IThemeColor) => {
        dispatch(theme_color(themeColor));
    };

    const slowChange = _.debounce((themeColor) => {
        updateColor(themeColor);
    }, 300);

    const colorChange = (themeColor: IThemeColor, isDebounce = false) => {
        if (isDebounce) {
            slowChange(themeColor);
        } else {
            updateColor(themeColor);
        }
    };
    ...
    return (
        <>
            <div className="d-flex align-items-center justify-content-between">
                <h5 className="mt-4 mb-3">Color Customization</h5>
                ...
                <div>
                    ...
                    <input
                        ...
                        onInput={(e: React.ChangeEvent<HTMLInputElement>) =>
                            colorChange({
                                value: "custom-color",
                                colors: { "--{{prefix}}primary": e.target.value }
                            },
                            true
                        )}
                        type="color"
                    ...
                    <input
                        ...
                        colorChange(
                            {
                                value: "custom-color",
                                colors: { "--{{prefix}}info": e.target.value },
                            },
                            true
                        )}
                        type="color"
